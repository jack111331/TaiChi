...
class Text2Motion_Transformer_Word_CrossAtt(nn.Module):
    ...
    def load_from_setting(setting_path, ckpt_type):
        ...
        if os.path.exists(ckpt_path):
            ...
            lpt_gpt.load_state_dict(ckpt['trans'], strict=False)
    ...
    def _mask_cond(self, cond, use_cfg=False):
    ...
    def forward(self, 
        ...
        clip_feature, word_emb, cond_mask = self._mask_cond((clip_feature, word_emb), use_cfg=use_cfg)
        if use_cfg:
    ...
    def sample_fast_nucleus(self, clip_feature, word_emb, word_length, cfg_scale=3, if_categorial=False, nucleus_p=0.85, use_kv_cache=False):
        ...
        for cur_pos in range(sample_max_len):
            ...
            logits_up, _ = outputs
            logits_up, aux_logits_up = logits_up[:b], logits_up[b:]
            ...
            logits_down, aux_logits_down = logits_down[:b], logits_down[b:]
...
class CrossCondTransBase_Word_CrossAtt(nn.Module):
    ...
    def forward(self,
        ...
        if idx_up.shape[1] == 0 and idx_down.shape[1] == 0:
            ...
            for att_module_up, att_module_down in zip(self.cross_att_upper, self.cross_att_lower):
                cross_attn_up_outputs = att_module_up(token_embeddings_up, word_emb, mask=token_word_mask_upper, output_attention=output_attention)
                token_embeddings_up = cross_attn_up_outputs[0]
                cross_attn_down_outputs = att_module_down(token_embeddings_down, word_emb, mask=token_word_mask_lower, output_attention=output_attention)
...
class CrossCondTransHead(nn.Module):
    ...
    def forward(self, x, cond_mask, kv_cache=None, cur_tokenlen=50, output_attention=False):
        N, T, C = x.size()
        upper_t = (T - 1) // 2 # T can be odd, so minus one and use division's nature of discarding can effectively find upper body tokens
        attn_dict = {"head_attn": []}
        for blk_idx, blk in enumerate(self.blocks):
            cur_kv_cache = kv_cache[blk_idx] if kv_cache is not None else None
