...
def transforms_global(anim):
    ...
    transforms : (F, J, 4, 4) ndarray
        ...
        each frame F and joint J
    """
    locals_mat = transforms_local(anim)
    globals_mat = transforms_blank(anim)

    globals_mat[:, 0] = locals_mat[:, 0] # Assign root joint transformation to world coordinate system

    # Transform children joints according to parent joints
    for i in range(1, anim.shape[1]):
        globals_mat[:, i] = transforms_multiply(globals_mat[:, anim.parents[i]], locals_mat[:, i])
