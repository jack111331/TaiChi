...
class Joint2BVHConvertor:
    def __init__(self, flip_horizontal_joint=False):
        self.template = BVH.load('./visualization/data/template.bvh', need_quater=True)
        self.template_offset = self.template.offsets.copy()
        # Sometimes the motion is perform in the flip motion (probably due to the original dataset preprocessing problem), so we provide flip_horizontal_joint to fix this
        if not flip_horizontal_joint:
            ...
            self.re_order = [0, 2, 5, 8, 11, 1, 4, 7, 10, 3, 6, 9, 12, 15, 14, 17, 19, 21, 13, 16, 18, 20]
            self.re_order_inv = [0, 5, 1, 9, 6, 2, 10, 7, 3, 11, 8, 4, 12, 18, 14, 13, 19, 15, 20, 16, 21, 17]
            self.end_points = [4, 8, 13, 17, 21]
    ...
    def convert(self, positions, filename, iterations=10, foot_ik=True):
        ...
        # TODO should first turn the template into the correct direction
