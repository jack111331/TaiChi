...
def get_args_parser():
    ...
    parser.add_argument("--n-head-gpt", type=int, default=8, help="nb of total heads")
    parser.add_argument("--n-global-head", type=int, default=4, help="nb of global heads")
    parser.add_argument("--n-local-head", type=int, default=2, help="nb of local heads")
    parser.add_argument("--n-mix-head", type=int, default=2, help="nb of mix heads")
    parser.add_argument("--ff-rate", type=int, default=2, help="feedforward size")
    parser.add_argument("--drop-out-rate", type=float, default=0.1, help="dropout ratio in the pos encoding")
    parser.add_argument("--cond-drop-prob", type=float, default=0.1, help="Classifier-free guidance conditional dropout rate")
    parser.add_argument("--recep-field-len", type=int, default=4, help="Receptive field length for local head attention")

    parser.add_argument('--pkeep', type=float, default=1.0, help='keep rate for gpt training')

    parser.add_argument('--pkeep-part-predict', type=float, default=0.55, help='choose rate to predict only upper or lower')
    parser.add_argument('--cfg-scale', default=3, type=int, help='classifier-free guidance strength')
    parser.add_argument('--nucleus-p', default=0.85, type=int, help='Nucleus sample p value')
    ...
    ## resume
    parser.add_argument("--vq-resume-setting-pth", type=str, default=None, help="PM-VQ-VAE setting's resume path")
    parser.add_argument("--lptgpt-resume-setting-pth", type=str, default=None, help="LPT-GPT setting's resume path")
    
    
    ## output directory 
    parser.add_argument('--text_prompt', type=str, default='', help='text prompt')
    parser.add_argument('--out_dir', type=str, default='output_GPT_Final/', help='output directory')
    parser.add_argument('--exp-name', type=str, default='exp_corrupt_train_test_discrepancy', help='name of the experiment, will create a file inside out-dir')
    ...
    parser.add_argument('--ckpt-types', type=str, default=["best_fid"], nargs="+", choices=['best_fid', 'best_top1', 'best_mm_dist', 'best_div'], help='Save checkpoint according to what metrics')
